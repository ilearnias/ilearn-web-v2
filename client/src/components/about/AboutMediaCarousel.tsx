...
const AboutMediaCarousel = () => {
  ...
  useEffect(() => {
    if (mediaItems && mediaItems.length > 0) {
      console.log(
        "About Media Carousel - All media items:",
        mediaItems.map(
          (item) => `${item.id}-${item.title}-${item.displayOrder}`
        )
      );

      // Make sure media item with ID 53 (VM4v8Yk7nN0) is included
      console.log(
        "Looking for video ID 53:",
        mediaItems.find((item) => item.id === 53)
      );
      ...
      console.log(
        "About Media Carousel - Filtered items:",
        filtered.map((item) => `${item.id}-${item.title}-${item.displayOrder}`)
      );

      // Log the first 5 videos that will appear in the carousel
      console.log(
        "About Media Carousel - First 5 videos:",
        filtered
          .slice(0, 5)
          .map(
            (item) =>
              `${item.id}-${item.title}-${item.displayOrder}-${
                item.mediaUrl?.includes("VM4v8Yk7nN0") ? "NEW VIDEO!" : ""
              }`
          )
      );
  ...
  const getMediaType = (item: MediaItem): "video" | "image" => {
    ...
    if (
      (item.type as string) === "portrait-video" ||
  ...
  const renderMediaCard = (item: MediaItem) => {
    ...
    const transitionClass =
      "transition-all duration-300 hover:-translate-y-1 group relative cursor-pointer";
      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center mb-10">
          ...
          <p className="text-neutral-600 mt-3">
            Our presence across media platforms
          </p>
        </div>
            ...
            <div className="relative w-full h-0 pb-[56.25%] overflow-hidden">
              <iframe
                src={
                  ...
                  getYoutubeEmbedUrl(
                    selectedVideo.mediaUrl || selectedVideo.url || ""
                  ) ||
                  ""
                }
